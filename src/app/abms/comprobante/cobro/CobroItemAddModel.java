...
public class CobroItemAddModel extends AbstractTableModel {
	...
	public Object getValueAt(int column, int row) {
		...
		ReciboCobroItem currentObject = (ReciboCobroItem) dataList.get(row);
		...
		try {
			direccion_inmueble = currentObject.getContratoNovedadCobro().getContrato().getInmueble().getDireccion_completa();
			periodo_cuota = currentObject.getContratoNovedadCobro().getPeriodoCuota();
			
			if (currentObject.getIdItemTipo() != NovedadTipo.IVA) 
				monto_novedad = moneda.format(currentObject.getContratoNovedadCobro().getSaldo());
...
//		}
		...
		String tipo = NovedadTipo.getDescripcion(currentObject.getIdItemTipo());
//		if (currentObject.getIdItemTipo() == ItemTipo.ItemImpuestos) {
