...
public class ItemEditView extends ABMAddView {
    ...
    private ReciboPagoItem oReciboPagoItem;
    private PagoAddView refClaseLlama;
    private int index;
    ...
    public ItemEditView (PagoAddView refClaseLlama, ReciboPagoItem oReciboPagoItem, int index) {
        ...
        this.oReciboPagoItem = oReciboPagoItem;
        this.index = index;
        
        this.setModal(true);
        
        // Evaluo si estoy en un ALTA o en una EDICION
        if (oReciboPagoItem == null) {
		...
		if (oReciboPagoItem != null) {
			...
			if (oReciboPagoItem.getIdItemTipo() == ItemTipo.ItemImpuestos) {
			...
			try {
				direccion_inmueble = 	oReciboPagoItem.getContratoNovedadPago().getContrato().getInmueble().getDireccion_completa() + " / " + 
										oReciboPagoItem.getContratoNovedadPago().getContrato().getInmueble().getLocalidad().getDescripcion();
	...
	public void actionPerformed(ActionEvent e) {    	
    	...
    	if (e.getActionCommand().equals("ok")) {
    		...
    		if (this.oReciboPagoItem != null) {
    			...
    			/* Valido el monto del item no supere el total del mismo -- como puede haber valores NEGATIVOS aplico Valor Absoluto */
    			double monto_max = oReciboPagoItem.getMonto();
    			if ( Math.abs(total_item) > Math.abs(monto_max)) {
    			...
    			// Valido que no le modifique el SIGNO - 15/01/2014
    			} if ( ((total_item > 0) && (monto_max < 0)) || ((total_item < 0) && (monto_max > 0)) ) {
    				
    				CPPrincipal.abrirVentana(new FWWindowPaneMensajes("Error Edición Items","No se puede cambiar el SIGNO del item.",(short)1));
    				return;
    				
    			} else {
    			
    				oReciboPagoItem.setDescripcion(tLeyenda.getText());
    				oReciboPagoItem.setMonto(total_item);
	    			//oReciboCobroItem.getContratoNovedadCobro().setMonto(total_novedad);
	    		
	    			refClaseLlama.oModel.getDataList().set(index, oReciboPagoItem);
	    			refClaseLlama.update(refClaseLlama.oModel, true);
    			...
    			this.oReciboPagoItem = new ReciboPagoItem();
    			
    			oReciboPagoItem.setDescripcion(tLeyenda.getText());
    			oReciboPagoItem.setMonto(total_item);
    			
    			try {
    				oReciboPagoItem.setIdItemTipo(Short.parseShort(cboItemTipo.getSelectedId()));
				} catch (Exception e1) {
					e1.printStackTrace();
					// revienta como un sapoooo
				}
				
				refClaseLlama.oModel.getDataList().add(oReciboPagoItem);
				refClaseLlama.update(refClaseLlama.oModel, true);
